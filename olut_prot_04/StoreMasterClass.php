 ...
 *    2005/10/11  ver 1.00.01 Fix "DB unkonwn error" when store close date is not filled.
 *    2005/10/15  ver 1.00.02 Fix close date quote when add/modify data. 
 *    2005/10/16  ver 1.00.03 isdcd was not shown when "next" button.
 *
 */
...
class Store extends OlutApp
{
    ...
    function addEntry($formvars)
    {
        ...
        {
            $close_date = "'" . OlutApp::formatDate($formvars['close_date']) . "'";  // fix 2005/10/15
        }
        
        if(strlen($formvars['open_date'])==0)  // fix 2005/10/11
        {
            $open_date = 'null';
        }
        else 
    ...
    */
    ...
    function getNextEntry($formvars) {
        ...
        {
            // 2005/10/16 isdcd が無かったので追加。
            $_query =  sprintf("select code,ckd,scode,sckd,name,name_k,isdcd,tel,fax,zip,address,print_flag,to_char(open_date,'YYYYMMDD') as open_date,to_char(close_date,'YYYYMMDD') as close_date,mail_address,web_url from m_store where code > '%s'  and deleted is null order by code asc",$id);
        }
