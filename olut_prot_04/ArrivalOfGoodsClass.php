...
class ArrivalOfGoods extends OlutApp
{
            ...
            if(strlen($commodity_code) > 0){
                ...
                //{
                    if($this->amount[$i] != 0)
                    {
                        // 単価入力が無いので、逆算する。 
                        // 小数点以下は切り捨て。
                        $this->unit_price[$i] = number_format(floor($this->total_price[$i] / $this->amount[$i]));  
                    }
                // }
    ...
    */
    ...
    function getTargetSlipNo($formvars)
    {
        ...
        {
            $this->error = "伝票を選択してください";
            return false;
        }
        else
        {
            // select_yyyymmdd0000000 の形式でデータがポストされてくる。
            $tmp = ereg_replace("select_","",$formvars['arg']);
            $tmp_array = split(',',$tmp);
            $this->act_date  = $tmp_array[0];
            $this->slip_no = $tmp_array[1];
        }
        return true;
    }
    ...
    */
    function getModifyData()
    {
        ...
        $_query .= " where t.slip_no=$this->slip_no and t.act_date='$this->act_date'";
        ...
        foreach($this->sql->record as $rec)
        {
            ...
            $this->unit_price[$index]     = number_format($rec[11]);
    ...
    */
    ...
    function find()
    {
        // added this check 2005/10/12
        if(strlen($this->date_from) > 0 && strlen($this->date_from) != 8)
        {
            $this->error ="日付けは8桁で入力してください";
            return false;
        }
            
        if(strlen($this->date_to) > 0 && strlen($this->date_to) != 8)
        {
            $this->error ="日付けは8桁で入力してください";
            return false;
        }     
        ...
        $_query .= " order by t.act_date desc, t.slip_no";
