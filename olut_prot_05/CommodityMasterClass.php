 ...
 *    2005/12/13 ver 1.00.01 1.先頭の一文字のみ全角カナ、をチェックするように仕様変更。
 *                           2.「減耗フラグ」の表示、あり、なし、が逆。
 *                           3. 現在単価の入力チェックを追加。
 *
 */
...
class Commodity extends OlutApp
{
    ...
    function isValidForm($formvars) {
        ...
        // 全角カナ？
        // 2005/12/13
        // 先頭の一文字のみ全角カナ、をチェックするように仕様変更。
        $tmp = substr($formvars['name_k'],0,2);
        //
        if(!IsZenkakuKana($tmp)){
        ...
        // 2005/12/13
        // 現在単価の入力チェックを追加。
        if(strlen($formvars['current_unit_price'])==0)
        {
            $this->error = "現在単価を入力してください。";
            return false;
        }
    ...
    {
        // 2005/12/13 あり、なし、が逆でした。
        $values = array('0' =>'減耗計算あり', '1' => '減耗計算なし');
