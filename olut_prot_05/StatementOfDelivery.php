...
*    2005/10/26 ver 1.00.01 パラメータ指定を追加。
*
*/
...
else
{
    ...
    if( $login->validateUser($_POST) == true)
    {
        ...
        if($codes != null)
        {
            ...
            $process_target = $_SESSION['process_target'];

            // 以下だと、win32のapacheが落ちていた。
            // session_unset();
            // 以下で代替。
            $_SESSION['STATEMENT_OF_DELIVERY'] = array();

            // 再設定。
            $_SESSION['user_id'] = $uid;
            $_SESSION['screen_lines']   = $screen_lines;
            $_SESSION['process_target'] = $process_target;
            //
        }
        else
        {
