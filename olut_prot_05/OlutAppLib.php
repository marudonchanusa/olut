 ...
 *    2005/10/05  ver 1.00.01 added getTargetYearList, getTargetMonthList.
 *    2005/10/15  ver 1.00.02 added 'や' tranlslation.
 *
 */
...
class OlutApp
{
    ...
    *  前日。
    */
    function getPreviousDateWithoutSlashes($year,$month,$date)
    {
        $prev_date = mktime (0,0,0,$month,$date-1,$year);
        $year  = date('Y',$prev_date);
        $month = date('m',$prev_date);
    ...
    function getTargetYearList($selected_year = null)
    {
    ...
    function getTargetMonthList($selected_month = null)
    {
        if($selected_month == null)
        {
            $year  = null;
            $month = null;
            $this->getCurrentProcessDate($this->sql,&$year,&$month);
    ...
    */
    function translateWhere($letter_from)
    {
        ...
        if(!strncmp($letter_from, 'や',2))           // fix 2005/10/15
        {
            return "name_k >= 'ヤ' and name_k < 'ラ'";
        }
