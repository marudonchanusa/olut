 ...
 *    2005/10/06  ver 1.00.01 Set maxinum slip number as initial.
 *    2005/10/08  ver 1.00.02 fixed bug of memo display problem.
 *
 */
...
class ShipmentDisplay extends OlutApp
{
    ...
    function renderScreen($formvars)
    {
        ...
        $this->tpl->assign('memo',          $this->memos);   // fix 2005/10/08
        $this->tpl->assign('grand_total',   $this->grand_total);
        
        if($this->commodity_codes != null && count($this->commodity_codes))
        {
            $this->tpl->assign('print_disabled','');
        }
        else 
        {
            $this->tpl->assign('print_disabled','disabled');
        }
    ...
    {
        $_query = "select max(slip_no) from t_main where (act_flag='5' or act_flag='6' or act_flag='7') and deleted is null";
