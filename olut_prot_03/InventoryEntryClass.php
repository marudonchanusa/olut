...
class InventoryEntry extends OlutApp
{
    ...
    */
    ...
    function getInventoryList($formvars=array())
    {
        ...
        for($n=0; $n<$this->commodity_count; $n++)
        {
            ...
            // 入庫金額のサブ・クエリ
            $_query .= " (select sum(total_price) from t_main where act_flag='1' ";
            $_query .= " and act_date>='$target_date_from' and act_date<='$target_date_to' and com_code=t.com_code),";

            // 入庫数のサブ・クエリ
            $_query .= " (select sum(amount) from t_main where act_flag='1' and com_code=t.com_code";
            ...
            if( $this->sql->query($_query,SQL_INIT) == true )
            {
                ...
                if($this->sql->record != null )
                {
                    ...
                    $this->total_price[$n]     = trim($this->amounts[$n]*$this->unit_price[$n]);
                }
            }
            else
            {
    ...
    *  在庫レコードの更新
    */
    function updateInventory($code,$amount,$stock_price,$next_month,$calc_flag)
    {
    ...
    *  新規在庫レコードの挿入
    */
    function insertInventory($code,$amount,$stock_price,$next_month,$calc_flag)
    {
        if(isset($amount) and strlen($amount)>0)
        {
    ...
    */
    function saveNewInventory($formvars)
    {
        ...
        return $this->insertInventory($commodity_code,$inventory_amount,$inventory_price,$next_month);
    }
