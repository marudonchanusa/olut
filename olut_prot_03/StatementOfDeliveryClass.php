...
class StatementOfDelivery extends OlutApp
{
    ...
    function printOut($codes)
    {
        if(is_array($codes))
        {
            ...
            foreach($codes as $code)
            {
                if($code != null)
                {
                    if(!$this->printStatementOfDelivery($code,$dt))
                    {
                        return false;
                    }
                }
    ...
    function printStatementOfDelivery($code,$dt)
    {
        if($this->getShipmentData($code,$dt))
        {
            //
            $this->preparePDF();
            //
    ...
    *   出荷データを得る。
    */
    function getShipmentData($code,$dt)
    {
        ...
        $_query .= " where t.slip_no='$code' and t.act_date>='$dt' and t.com_code=c.code and u.code=c.unit_code";
        ...
        if( $this->sql->record != null )
        {
            ...
            foreach($this->sql->record as $rec)
            {
                if($y_index == 1)
                {
                    ...
                    $this->pdf->Write(10,"配送日付 $year 年 $month 月 $day 日");
                }
                $this->pdf->SetFont(GOTHIC,'',11);
                ...
                $this->WriteCell(5,$y_index,$rec[6],$y);       // memo
