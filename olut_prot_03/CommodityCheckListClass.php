...
*
*   Program name:
*    取引先チェックリスト - CommodityCheckListClass.php
*
*   Release History:
...
class CommodityCheckList_Smarty extends Smarty
{
    function CommodityCheckList_Smarty()
    {
...
class CommodityCheckList_SQL extends SQL
{
    function CommodityCheckList_SQL()
    {
        //
...
class CommodityCheckList extends OlutApp
{
    var $sql;
    var $tpl;
    ...
    var $target_year;
    var $target_month;
    var $number_of_lines_per_page = 44;

    function CommodityCheckList()
    {
        $this->sql =& new CommodityCheckList_SQL();
        $this->tpl =& new CommodityCheckList_Smarty();
    }
    ...
    function printOut($formvars)
    {
        ...
        //
        $_query  = "select c.code, c.name, c.current_unit_price, c.next_unit_price, u.name from m_commodity c, m_unit u";
        $_query .= " where u.code=c.unit_code ";
        $_query .= " order by c.code";
    ...
    function printHeader()
    {
        ...
        $this->pdf->Write(10,"商品名称");

        $this->pdf->SetXY(100,30);
        $this->pdf->Write(10,"現在単価");

        $this->pdf->SetXY(120,30);
        $this->pdf->Write(10,"来月単価");
        
        $this->pdf->SetXY(140,30);
        $this->pdf->Write(10,"単位");
        //
    ...
    function printLine()
    {
        ...
        $this->pdf->SetXY(20,$this->current_y);
        $this->pdf->Write(20,$code);

        $this->pdf->SetXY(35,$this->current_y);
        $this->pdf->Write(20,$name);

        $this->pdf->SetXY(100,$this->current_y);
        $this->pdf->Write(20,OlutApp::formatNumber($this->sql->record[2],8,0));
        
        $this->pdf->SetXY(120,$this->current_y);
        $this->pdf->Write(20,OlutApp::formatNumber($this->sql->record[3],8,0));

        $this->pdf->SetXY(140,$this->current_y);
        $this->pdf->Write(20,$this->sql->record[4]);
